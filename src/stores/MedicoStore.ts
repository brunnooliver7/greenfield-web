...
class MedicoStore {
  ...
  salvar = (medico: Medico, onSuccess: () => void) => {
    axios
      .post('http://localhost:8080/medico', medico)
      .then(() => {
        toast('Médico salvo!', medicoSalvoToastOptions)
        onSuccess()
      })
      .catch(() =>
        toast('Erro ao tentar salvar dados do médico', medicoFalhaToastOptions)
      )
  }

  editar = (medico: Medico, onSuccess: () => void) => {
    axios
      .put('http://localhost:8080/medico', medico)
      .then(() => {
      ...
      .catch(() =>
        toast(
          'Erro ao tentar atualizar dados do médico',
          medicoFalhaToastOptions
        )
      )
      .finally(() => {
  ...
  obterMedicos = async () => {
    ...
    await fetch('http://localhost:8080/medico')
      ...
      .catch(() =>
        toast('Erro ao tentar obter lista de médicos', medicoFalhaToastOptions)
      )
