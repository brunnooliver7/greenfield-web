...
const MedicoForm = () => {
  const { salvar, editar, formMode, setFormMode, medico } =
    useContext(MedicoStore)
  ...
  const preencherCamposModoEdicao = (medico: Medico) => {
    ...
    setValue(
      'estado_registro_crm',
      Object.values(EstadosRegistroCRM).find(
        (e) => e.toLowerCase() === medico.estado_registro_crm.toLowerCase()
      )
    )
    ...
    <>
      ...
      <div className="overflow-y-scroll scrollbar">
        <div className="flex justify-center items-center h-full">
          <div
            id="medico-form"
            className="w-96 rounded-full flex justify-center items-center p-5"
          >
              ...
              <form onSubmit={handleSubmit(onSubmit)}>
                <div className="grid grid-rows-1 gap-4">
                  ...
                  <div>
                    <Controller
                      ...
                      render={({
                        ...
                        <Select
                          ...
                          onChange={(estado_registro_crm) =>
                            controllerOnChange(estado_registro_crm)
                          }
                          error={Boolean(errors?.estado_registro_crm)}
                        >
                          {Object.values(EstadosRegistroCRM).map(
                            (estado_registro_crm) => (
                              <Option
                                key={estado_registro_crm}
                                value={estado_registro_crm}
                              >
                    ...
                    />
                    {errors?.estado_registro_crm && (
                      <span className="text-xs text-red-600">
                        {errors?.estado_registro_crm?.message?.toString()}
                      </span>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {errors?.dt_nascimento && (
                      <span className="text-xs text-red-600">
                        {errors?.dt_nascimento?.message?.toString()}
                      </span>
                    )}
                  </div>
...
const medicoFormValidationSchema = yup.object().shape({
  ...
  estado_registro_crm: yup
    .string()
    .required('Informe um estado de registro do CRM'),
  sexo: yup.string().required('Informe um sexo'),
  dt_nascimento: yup.string().required('Informe uma Data de Nascimento'),
  email: yup
    .string()
    .email('Informe um email válido')
    .required('Informe um Email'),
  estado: yup.string().required('Informe um Estado'),
